...
// extendedConfigCacheAdapter wraps SyncMap to implement ExtendedConfigCache
type extendedConfigCacheAdapter struct {
...
func (c *callbackVFS) Chtimes(path string, aTime time.Time, mTime time.Time) error {
	return c.osvfs.Chtimes(path, aTime, mTime)
}
