...
extension ESBuildPlugin {
    ...
    static func deserializePluginData(_ json: String?) -> Any? {
...
extension ESBuildOnResolveResult {
    ...
    var cValue: UnsafeMutablePointer<c_on_resolve_result> {
        ...
        if !watchFiles.isEmpty {
            result.pointee.watch_files = UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>.allocate(capacity: watchFiles.count)
        ...
        if !watchDirs.isEmpty {
            result.pointee.watch_dirs = UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>.allocate(capacity: watchDirs.count)
